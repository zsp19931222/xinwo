...
public class SeekBarPressure extends View {
	...
	public OnSeekBarPressureClickListener lister;
	/*
	...
	private int mThumbMarginTop = 60; // 滑动块顶部距离上边框距离，也就是距离字体顶部的距离

	private int mFlag = CLICK_INVAILD;
	private OnSeekBarChangeListener mBarChangeListener;
	private int max = 999;// 最大值
	...
	public SeekBarPressure(Context context, AttributeSet attrs, int defStyle) {
		...
		notScrollBarBg = resources.getDrawable(R.drawable.line);
		hasScrollBarBg = resources.getDrawable(R.drawable.line_pre);
		mThumbLow = resources.getDrawable(R.drawable.city_wide_sereen_count);
		mThumbHigh = resources.getDrawable(R.drawable.city_wide_sereen_count);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mOffsetHigh = formatDouble(defaultScreenHigh / 100 * (mDistance))
				+ mThumbWidth / 2;
		setMeasuredDimension(width + 55, mThumbHeight + mThumbMarginTop + 2);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		text_Paint.setColor(getResources().getColor(R.color.淡红色));
		...
		double progressLow = formatDouble((mOffsetLow - mThumbWidth / 2) * max
				/ mDistance);
		...
		canvas.drawText((int) progressLow + "", (int) mOffsetLow - 2 - 2, 40,
				text_Paint);
		if (progressHigh == max) {
			canvas.drawText((int) progressHigh + "+", (int) mOffsetHigh - 2,
					40, text_Paint);
		} else {
			canvas.drawText((int) progressHigh + "", (int) mOffsetHigh - 2, 40,
					text_Paint);
		}

		lister.send((int)progressHigh, (int)progressLow);
	...
	public interface OnSeekBarPressureClickListener {
		void send(int high,int low);
	}

	public void setOnSeekBarPressureClickListener(OnSeekBarPressureClickListener lister) {
		this.lister = lister;
	}
}
